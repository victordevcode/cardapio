                    ...
                    pedidosDaColuna.map((ped) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Botões de Ação para Movimentação Rápida */}
                          <div className="pt-3 mt-2 border-t border-gray-100 flex gap-2 justify-between items-center">
                            {colunaAnterior ? (
                              <button
                                onClick={() => moverStatus(ped.id, colunaAnterior.id)}
                                className="px-2.5 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded-md transition-colors flex items-center gap-1"
                                title={`Voltar para ${colunaAnterior.titulo}`}
                              >
                                ⬅ 
                              </button>
                            ) : (
                              <div /> // Div vazia para alinhar o botão "Avançar" à direita se for a primeira coluna
                            )}

                            {proximaColuna && (
                              <button
                                onClick={() => moverStatus(ped.id, proximaColuna.id)}
                                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-md shadow-sm transition-colors flex items-center gap-1 ml-auto"
                                title={`Mover para ${proximaColuna.titulo}`}
                              >
                                 ➡
                              </button>
                            )}
